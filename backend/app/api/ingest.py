...
async def ingest_graph(payload: Dict[str, Any] = Body(...)):
    """
    Ingests extracted nodes and relationships into FalkorDB domain-isolated graph.
    """
    ...
    domain = payload.get("domain") or graph_data.get("domain")
    result = cypher_builder.ingest_graph_data(graph_data, domain=domain)
    return result

@router.post("/batch-ingest")
async def batch_ingest(payload: Dict[str, Any] = Body(...)):
    """
    Batch processes dataset images from the project 'data/' directory into domain graphs.
    """
    target_category = payload.get("category", "Invoice")
    ...
    for img_path in image_files:
        try:
            ...
            ingest_res = cypher_builder.ingest_graph_data(graph_data, domain=domain_key)

            processed.append({
                "filename": filename,
                "status": "success",
                "graph_name": ingest_res.get("graph_name"),
